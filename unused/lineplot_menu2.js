...
function bigsearch() {
  ...
  var tol = document.getElementById("CS_tolerance"); //IS censhift
  //var Qshift_tol = document.getElementById("Qshift_tolerance"); //IS censhift
  //var Bhf_tol = document.getElementById("Bhf_tolerance"); //IS censhift

  var min3, filtermin3, filtermax3, td3; 
  min3 = document.getElementById("temp"); //TEMP
  filtermin3 = +min3.value-min3.value*tol.value/100;
  filtermax3 = +min3.value+min3.value*tol.value/100;
  //filtermin3 = min3.value;
  //filtermax3 = min3.value;


  var min4, filtermin4, filtermax4, td4; 
  min4 = document.getElementById("censhift_1"); //IS censhift
  filtermin4 = +min4.value-min4.value*tol.value/100;
  filtermax4 = +min4.value+min4.value*tol.value/100; //Need to make sure both values are not strings so just put a + in front of both terms
  ...
  filtermin5 = +min5.value-min5.value*tol.value/100;
  filtermax5 = +min5.value+min5.value*tol.value/100;

  var min7, filtermin7, filtermax7, td7; 
  min7 = document.getElementById("hyperfinefield_1"); // hyperfinefield
  filtermin7 = +min7.value-min7.value*tol.value/100;
  filtermax7 = +min7.value+min7.value*tol.value/100;
  ...
  // Loop through all table rows, and hide those who don't match the search query
  for (i = 0; i < tr.length; i++) {
    td3 = tr[i].getElementsByTagName("td")[2];    //Temp
    td4 = tr[i].getElementsByTagName("td")[3];    //center shift
    td5 = tr[i].getElementsByTagName("td")[4];    //Quadrupole shift
    td7 = tr[i].getElementsByTagName("td")[6];    // Hyperfine field
    
    if (min7.value>0){
        if (td3 && td4 && td5 && td7) {
          if (
            (td4.innerHTML >= filtermin4 
            && td4.innerHTML <= filtermax4
            
            && td3.innerHTML >= filtermin3 
            && td3.innerHTML <= filtermax3  
...
function bigsearch2() {
  ...
  var min3, filtermin3, filtermax3, td3; 
  min3 = document.getElementById("temp"); //IS censhift
  //filtermin3 = min3.value-min3.value*tol.value/100;
  //filtermax3 = min3.value+min3.value*tol.value/100;
  filtermin3 = min3.value;
  filtermax3 = min3.value;
  ...
  //max4 = min4*0.05; 
  filtermin4 = min4.value-min4.value*tol.value/100;
  filtermax4 = min4.value+min4.value*tol.value/100;
...
function Loren_sum(x) {
	...
	function draw_sum() {
			var canvas = document.getElementById("canvas");
			//var xscale = document.getElementById("slider2").value;
			var xscale=41.5; //This value dictates the width of the fit. Needs to be calibrated
      ...
      var xaxis = 1*canvas.width;
      var xaxis0 = 0*canvas.width;
...
function samplespectra(){
  ...
  /*var canvas = document.getElementById("canvas");
  var ctx = canvas.getContext("2d");
  var checkBox1 = document.getElementById("tick1");
  var checkBox2 = document.getElementById("tick2");
  var checkBox3 = document.getElementById("tick3");
...
//For loading pre-installed data
function processlistData() {
    ...
    var deliminat5 = document.getElementById("delChoice5");
...
function processData(csv) {
    ...
    //var refinfo = allTextLines;
    //refinfo.splice(5,allTextLines.length);
    //document.getElementById('referenceinfo').innerHTML = refinfo;
...
//Draws the box for the axes
    ...
    var xminlabel=-12
    var xmaxlabel=12
    var xrange=canvas.width*1/24*2;
    var yrange=canvas.height*0.9;

    var xrangemin = document.getElementById("rangemin").value;
    var xrangemax = document.getElementById("rangemax").value;
    var xxrange=xrangemax-xrangemin;
    var xtab=canvas.width*1/xxrange*2;
    ...
    ctx.font="13pt Arial";
    ctx.fillText("Velocity (mm/s)",canvas.width/2-48,yrange+30);
...
function MBdata(lines){
  ...
  if(deliminat1.checked){
    ...
    for(i=0;i<names.length;i++){
      listx = listx.concat(names[i][2] + names[i][3] + names[i][4] + names[i][5] + names[i][6] +','); //Combines the values of the first (if 0) column into a string
      listy = listy.concat(names[i][7] + names[i][8] + names[i][9] + names[i][10] + names[i][11] +','); //Combines the values of the second column (if 1) into a string
      listy = listy.replace("undefined", "");
    }
  }
...
function newGraph(){
  ...
  return x0*xrange*1-x00*xrange*1; });
  ...
  if(marker1.checked){
    for (var i = 0; i < x.length; i++) {
      ...
      ctx.fillRect(x[i]-1.5,y[i]+1.5,3,3); 
    ...
    for (var i = 0; i < x.length; i++) {
      ...
      ctx.beginPath();
      ctx.moveTo(x[i]-1.5,y[i]+1.5);
      ctx.lineTo(x[i+1]-1.5,y[i+1]+1.5);
      //ctx.stroke();
    };
  }
...
function MBspectra(lines){
  ...
  document.getElementById('outputtxt1b').value = listx;
  document.getElementById('outputtxt2b').value = listy;
...
function newGraphspectra(){
   
  var listx = document.getElementById("outputtxt1b").value;
  var listy = document.getElementById("outputtxt2b").value;
  var canvas = document.getElementById("canvas");
  var ctx = canvas.getContext("2d");
